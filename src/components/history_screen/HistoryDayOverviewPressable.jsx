...
export default function HistoryDayOverviewPressable({
  data,
  openModalAndSetData,
}) {
  const date = `${data.date.getDate()}.${data.date.getMonth() + 1}.${data.date.getFullYear()}`;
  const exercises = data.exercises.length;
  const diet = data.diet.length;
  const sleep = data.sleep.length;

  return (
    <Pressable
      style={styles.dayButton}
      onPress={() => openModalAndSetData(data)}
    >
      <View style={styles.row}>
        <CustomText style={styles.dayButtonText}>{date}</CustomText>
        <CustomText style={[styles.dayButtonText, { color: "#913737" }]}>
          Press to Edit
        </CustomText>
      </View>
      <View style={styles.row}>
        <CustomText style={styles.dayButtonText}>
          Exercises: {exercises}
        </CustomText>
        <CustomText style={styles.dayButtonText}>Diet: {diet}</CustomText>
